...
function getRangeLabel(range: TelemetryRange): string {
  const labels: Record<TelemetryRange, string> = {
  ...
  return labels[range] ?? 'Último año'
}
    ...
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-5">
        {(() => {
          ...
          if (isLoading) {
            return <div className="h-[520px] rounded-[1.25rem] bg-slate-100" />
          }
          return <AccessMap ecosystems={ecosystems} />
        })()}
      </div>
    </div>
  )
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-6 rounded-3xl border border-slate-200 bg-slate-50 p-5 text-center">
        <p className="text-sm text-slate-500">Rango actual: {getRangeLabel(range)}</p>
        <p className="mt-3 text-4xl font-semibold text-slate-900">{formatBytes(totalBytes)}</p>
      </div>
    </div>
  )
}
...
function GlobalStatusCard({
  ...
  canViewUserCount: boolean
  isAdminOrGlobalAdmin: boolean
  users: Array<{ id: string; email: string; name: string; role: string; status: string; createdAt: string }>
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      {(() => {
        if (telemetryError && telemetryError !== 'NO_DATA') {
          return (
...
function TransactionVolumeChart({
  ...
  changeRange: (r: TelemetryRange) => void
  data: { dailyVolume: Array<{ timestamp: string; tx: number }> }
}) {
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-4 text-slate-900">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="text-xl font-semibold text-slate-900">{getRangeLabel(range)}</h2>
          </div>
        </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data.ecosystemUsage} margin={{ top: 10, right: 18, left: 0, bottom: 30 }}>
              ...
              <Bar dataKey="anchors" radius={[4, 4, 0, 0]} fill="#7c3aed">
                {data.ecosystemUsage.map((entry, index) => (
                  <Cell key={entry.name} fill={index % 2 === 0 ? '#7c3aed' : '#a855f7'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid gap-6 xl:grid-cols-[1.4fr_0.6fr]">
          <section className="space-y-6">
            <EcosystemMapCard ecosystems={ecosystems} isLoading={isMapLoading} error={mapError} />
            <InfoVolumeCard range={range} totalBytes={totalInformationBytes} />
          </section>
          <section className="space-y-6">
            <GlobalStatusCard
              canViewUserCount={canViewUserCount}
              isAdminOrGlobalAdmin={isAdminOrGlobalAdmin}
              users={users}
        ...
        <div className="mt-6 space-y-6">
          <TransactionVolumeChart range={range} ranges={ranges} changeRange={changeRange} data={data} />
          <ChartsRow data={data} />
        </div>
      </div>
    </div>
